...
void EdsResourcesCacheImpl::removeResource(
    absl::string_view resource_name) {
...
EdsResourcesCacheImpl::getResource(
...
void EdsResourcesCacheImpl::removeCallback(
    absl::string_view resource_name, EdsResourceRemovalCallback* removal_cb) {
  if (const auto& resource_it = resources_map_.find(resource_name);
      resource_it != resources_map_.end()) {
...
uint32_t EdsResourcesCacheImpl::cacheSizeForTest() const {
  return resources_map_.size();
}

void EdsResourcesCacheImpl::setExpiryTimer(absl::string_view resource_name, std::chrono::milliseconds ms) {
  auto it = expiry_timers_.find(resource_name);
  if (it == expiry_timers_.end()) {
    // No timer for this resource, create one, and create a copy of resource_name that will outlive this function.
    Event::TimerPtr resource_timeout = dispatcher_.createTimer([this, str_resource_name = std::string(resource_name)]() -> void {
