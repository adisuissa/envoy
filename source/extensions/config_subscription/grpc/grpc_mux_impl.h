  ...
  struct GrpcMuxWatchImpl : public GrpcMuxWatch {
    GrpcMuxWatchImpl(const absl::flat_hash_set<std::string>& resources,
        ...
        : callbacks_(callbacks), resource_decoder_(resource_decoder), type_url_(type_url),
          ...
          watches_(parent.apiStateFor(type_url).watches_), eds_resources_cache_(eds_resources_cache) {
      updateResources(resources);
      if (eds_resources_cache_.has_value()) {
        ASSERT(type_url == Config::getTypeUrl<envoy::config::endpoint::v3::ClusterLoadAssignment>());
        ENVOY_LOG_MISC(trace, "ADIP: GrpcMuxWatchImpl() with eds_resources_cache");
      }
    }
  ...
  private:
    void updateResources(const absl::flat_hash_set<std::string>& resources) {
      ...
      if (eds_resources_cache_.has_value()) {
        ...
        std::set_difference(previous_resources.begin(), previous_resources.end(), resources_.begin(), resources_.end(), std::back_inserter(removed_resources));
    ...
    // Optional cache for the specific type_url (ClusterLoadAssignment) of this watch.
