...
// up to the specific xDS type subscription handler (e.g., EdsClusterImpl), to fetch
...
public:
  WatchMap(const bool use_namespace_matching, const std::string& type_url,
    ...
    // A cache can only be used for EDS resources watch-map.
    if (eds_resources_cache_) {
      ASSERT(type_url_ == Config::getTypeUrl<envoy::config::endpoint::v3::ClusterLoadAssignment>());
    }
  }
