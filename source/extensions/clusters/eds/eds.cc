...
EdsClusterImpl::EdsClusterImpl(const envoy::config::cluster::v3::Cluster& cluster,
    ...
    : BaseDynamicClusterImpl(cluster, cluster_context),
      ...
      eds_resources_cache_(Runtime::runtimeFeatureEnabled("envoy.restart_features.use_eds_cache_for_ads") ? cluster_context.clusterManager().edsResourcesCache() : absl::nullopt) {
...
void EdsClusterImpl::onConfigUpdate(const std::vector<Config::DecodedResourceRef>& resources,
  ...
  if (stale_after_ms > 0) {
    ...
    if (eds_resources_cache_.has_value()) {
      eds_resources_cache_->setExpiryTimer(edsServiceName(), std::chrono::milliseconds(stale_after_ms));
    }
  }
...
void EdsClusterImpl::update(const envoy::config::endpoint::v3::ClusterLoadAssignment& cluster_load_assignment) {
...
void EdsClusterImpl::onConfigUpdateFailed(Envoy::Config::ConfigUpdateFailureReason reason,
  ...
  if (reason == Envoy::Config::ConfigUpdateFailureReason::FetchTimedout &&
      eds_resources_cache_.has_value()) {
    ENVOY_LOG(trace, "onConfigUpdateFailed due to timeout for {}, looking for cached resources", edsServiceName());
    auto cached_resource = eds_resources_cache_->getResource(edsServiceName(), this);
    if (cached_resource.has_value()) {
      ENVOY_LOG(debug, "Did not receive EDS response on time, using cached ClusterLoadAssignment for cluster {}", edsServiceName());
