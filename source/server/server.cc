...
void InstanceImpl::initialize(const Options& options,
  ...
  *bootstrap_.mutable_node()->mutable_user_agent_max_api_version() = VersionInfo::apiVersion();
  *bootstrap_.mutable_node()->mutable_user_agent_min_api_version() =
      VersionInfo::oldestApiVersion();
  for (const auto& ext : Envoy::Registry::FactoryCategoryRegistry::registeredFactories()) {
