...
AdsIntegration2Test::AdsIntegration2Test(const envoy::config::core::v3::ApiVersion api_version)
    : HttpIntegrationTest(
...
void AdsIntegration2Test::TearDown() { cleanUpXdsConnection(); }

envoy::config::cluster::v3::Cluster
AdsIntegration2Test::buildCluster(const std::string& name, const std::string& lb_policy) {
  return ConfigHelper::buildCluster(name, lb_policy, api_version_);
}

envoy::config::cluster::v3::Cluster AdsIntegration2Test::buildTlsCluster(const std::string& name) {
  return ConfigHelper::buildTlsCluster(name, "ROUND_ROBIN", api_version_);
}

envoy::config::cluster::v3::Cluster
AdsIntegration2Test::buildRedisCluster(const std::string& name) {
  return ConfigHelper::buildCluster(name, "MAGLEV", api_version_);
}

envoy::config::endpoint::v3::ClusterLoadAssignment
AdsIntegration2Test::buildClusterLoadAssignment(const std::string& name) {
...
envoy::config::endpoint::v3::ClusterLoadAssignment
AdsIntegration2Test::buildTlsClusterLoadAssignment(const std::string& name) {
  return ConfigHelper::buildClusterLoadAssignment(
      name, Network::Test::getLoopbackAddressString(ipVersion()), 8443, api_version_);
}

envoy::config::listener::v3::Listener
AdsIntegration2Test::buildListener(const std::string& name, const std::string& route_config,
...
envoy::config::listener::v3::Listener
AdsIntegration2Test::buildRedisListener(const std::string& name, const std::string& cluster) {
...
AdsIntegration2Test::buildRouteConfig(const std::string& name, const std::string& cluster) {
  return ConfigHelper::buildRouteConfig(name, cluster, api_version_);
}

void AdsIntegration2Test::makeSingleRequest() {
  registerTestServerPorts({"http"});
  testRouterHeaderOnlyRequestAndResponse();
  cleanupUpstreamAndDownstream();
}

void AdsIntegration2Test::initialize() { initializeAds(false); }

void AdsIntegration2Test::initializeAds(const bool rate_limiting) {
...
void AdsIntegration2Test::testBasicFlow() {
...
envoy::admin::v3::ClustersConfigDump AdsIntegration2Test::getClustersConfigDump() {
  auto message_ptr =
...
envoy::admin::v3::ListenersConfigDump AdsIntegration2Test::getListenersConfigDump() {
  auto message_ptr =
...
envoy::admin::v3::RoutesConfigDump AdsIntegration2Test::getRoutesConfigDump() {
  auto message_ptr =
