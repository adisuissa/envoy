...
TEST_F(EdsAssignmentTimeoutTest, AssignmentLeaseExpired) {
  ...
  EXPECT_CALL(*interval_timer_, enabled()).Times(1);
  doOnConfigUpdateVerifyNoThrow(cluster_load_assignment);
  {
    auto& hosts = cluster_->prioritySet().hostSetsPerPriority()[0]->hosts();
    EXPECT_EQ(hosts.size(), 2);
...
// Test that assignment timeout is disabled when an update arrives.
...
public:
  ...
  void resetCluster(const std::string& yaml_config, Cluster::InitializePhase initialize_phase) {
    ...
    Envoy::Upstream::ClusterFactoryContextImpl factory_context(
        server_context_, server_context_.cluster_manager_, nullptr, ssl_context_manager_, nullptr,
        false);
    ON_CALL(server_context_.cluster_manager_, edsResourcesCache()).WillByDefault(
...
TEST_F(EdsCachedAssignmentTest, ClusterUpdateNotUsingCacheOnEdsUpdate) {
  ...
  EXPECT_CALL(*interval_timer_pre_, enabled()).Times(1);
  ...
  updateCluster();
  {
    ...
    EXPECT_CALL(*interval_timer_post_, enabled()).Times(1);
...
TEST_F(EdsCachedAssignmentTest, UseCachedAssignmentOnWarmingFailure) {
  ...
  cached_cluster_load_assignment.mutable_endpoints(0)->mutable_lb_endpoints(0)->mutable_load_balancing_weight()->set_value(22);
  ...
  updateCluster();
  {
    ...
    EXPECT_CALL(eds_resources_cache_, getResource("fare", _)).WillOnce(Return(cached_cluster_load_assignment));
    ENVOY_LOG_MISC(trace, "ADIP: testsing - here3");
    eds_callbacks_post_->onConfigUpdateFailed(Envoy::Config::ConfigUpdateFailureReason::FetchTimedout,
                                         &dummy_ex);
    const auto& hosts = cluster_post_->prioritySet().hostSetsPerPriority()[0]->hosts();
    EXPECT_EQ(hosts.size(), 1);
...
TEST_F(EdsCachedAssignmentTest, CachedAssignmentUpdate) {
  ...
  // TODO(adip): add timer and see that it is disabled in the cache.
  initialize();
  // Expect the timer to be enabled once.
  ...
  {
    EnvoyException dummy_ex("dummy exception");
    EXPECT_CALL(eds_resources_cache_, getResource("fare", _)).WillOnce(DoAll(SaveArg<1>(&removal_cb), Return(cached_cluster_load_assignment)));
    eds_callbacks_post_->onConfigUpdateFailed(Envoy::Config::ConfigUpdateFailureReason::FetchTimedout,
                                         &dummy_ex);
    const auto& hosts = cluster_post_->prioritySet().hostSetsPerPriority()[0]->hosts();
    EXPECT_EQ(hosts.size(), 1);
